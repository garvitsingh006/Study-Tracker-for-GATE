...
import { ChevronDown, ChevronUp, CheckCircle2, Circle, Clock, RotateCcw, Plus, X, ExternalLink } from 'lucide-react'
        ...
        <div style={{ paddingLeft: '34px', paddingBottom: '12px' }}>
          {subMaterials.map(mat => (
            <div key={mat.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <button onClick={() => onRemoveMaterial(subtopic.id, mat.id)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', flexShrink: 0, padding: '0' }}>
                <X size={12} />
              </button>
            </div>
          ))}
  ...
  return (
    <div className="bento-card" style={{ marginBottom: '12px', padding: 0, overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{ width: '64px', height: '4px', background: 'var(--border-subtle)', borderRadius: '2px', overflow: 'hidden' }}>
